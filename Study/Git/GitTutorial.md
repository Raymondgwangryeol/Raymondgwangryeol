...
### Git 브랜치란?
독립적인 개발 라인으로, 새 기능이나 버그 수정 등의 작업을 할때 작업별로, 혹은 팀원 별로 작업을 분리할 수 있다.
협업 환경에서 base가 되는 코드에서 브랜치를 생성해 작업을 다른 사람과 분리하거나 여러 버전을 관리할 수 있다.(병렬 수행 가능)
또, 나중에 코드 베이스에 다시 병합하는데도 수월하다.
각 라인은 독립적이기 때문에, 커밋이 병합되는 등 다른 곳에서 변경 사항을 가져오지 않는 한 다른 브랜치에 영향을 미치지 않는다.
각 작업마다 브랜치를 만들면, 변경 사항을 쉽게 식별하고, 역추적을 간소화 할 수 있다.

### Git branch 만들기 - git branch 명령어 사용
git branch [만들고자 하는 브랜치 이름] 명령으로, 새 branch를 생성할 수 있다.
만약 아래와 같이 git branch issue1으로 브랜치를 생성하면, 현재 커밋을 가리키는 issue1 포인터가 추가된다.

### 브랜치 전환 - git checkout
브랜치 전환시, 해당 브랜치에 저장되있던 버전과 작업 트리가 보여지게 된다.

### 브랜치 가리키기
HEAD는 브랜치의 현재 스냅샷을 가리켜 사용자에게 알려준다.
HEAD가 특정 스냅샷을 가리키면, 해당 스냅샷이 속한 브랜치가 활성화된다.
새로 저장소를 만들었을 때, HEAD는 자동으로 main 브랜치를 가리키고, HEAD가 다른 스냅샷을 가리키면 활성 브랜치가 업데이트 된다.

[HEAD 상대 참조]
로그를 취소하거나 삭제하는 등 커밋 참조할 때 HEAD의 상대참조 기호인 ~,^를 써서 범위를 정한다.
~num: HEAD제외 num개의 조상을 나타냄
^num: 한 번에 한 개의 조상만을 나타내며, 부모가 여러개일 경우 num번째 부모를 나타냄.

### 브랜치 보관
만약 A 브랜치에 변경사항이 있는데, 갑자기 새로운 브랜치로 전환하면, A에서 커밋되지 않은 변경사항은 새로운 브랜치로 전달된다(아... 어쩐지...)
하지만, 커밋되지 않은 변경사항과 전환하려는 새 브랜치 파일 간의 충돌이 일어나면 브랜치 전환이 허용되지 않는다.
커밋 안 된 변경사항 잠시 어디다가 숨겨놨다가 다시 꺼내면 안되냐..? -> git stash를 이용하세요!
stach에 저장해 놓고 클린한 상태에서 브랜치 전환 가능. 필요하면 언제든 꺼내 쓸 수 있다.
