...
### 로컬 저장소 생성 방법
1. git init으로 원하는 폴더에서 아예 새로운 저장소를 만들기</br>
   => version이 저장되지 않은 기존 프로젝트에 git을 도입, 변경사항 추적 시작 가능</br>
2. git clone으로 원격 저장소를 로컬 시스템에 복사하기</br>
   => 복제된 원격 main branch를 추적하는 local main branch를 자동으로 설정.</br>
   => 복제된 로컬 저장소에는 원본과 동일한 기록 로그 파일이 있음. 원격 저장소에서 로컬 저장소의 모든 커밋을 참조하고 역추적할 수 있음.</br>
</br></br></br>
